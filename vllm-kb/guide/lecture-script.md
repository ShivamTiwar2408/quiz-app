...
Okay. So the model reads all your tokens and predicts the single most likely next token. Then — and this is the important part — it **appends** that new token to your text, and does the whole thing again. It reads everything, including the token it just produced, and predicts the next one. And again. And again. This loop is called **autoregression**, and it keeps going until the model produces a special "I'm finished" token.
...
Let me make that concrete. Picture a GPU's memory as a plot of land that several tenants have to share. Take one NVIDIA A100 with 40 gigabytes of VRAM, running a 13-billion-parameter model. Roughly 26 gigabytes goes to the model's weights — that's the fixed "brain" of the model, and we can't shrink it. Another couple of gigabytes goes to system overhead. That leaves about 12 gigabytes. And *that* 12 gigabytes is the only part we get to manage cleverly. It's used for something called the KV cache, which we'll meet in a moment.
...
Inside the model, when it "reads" a token, it computes two things for that token: a **Key** and a **Value**. You don't need the math. You just need one fact: a token's Key and Value never change once they're computed. They're fixed.

So the fix is almost embarrassingly obvious once you see it: **compute each token's Key and Value once, then just save them.** That saved store is called the **KV cache** — K for key, V for value. Now, when the model generates a new token, it only has to compute the Key and Value for that one new token, and it reuses everything else straight from the cache.
