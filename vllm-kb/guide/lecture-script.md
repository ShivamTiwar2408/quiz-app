...
To predict the next token, the model uses a mechanism called **attention** — each new token "looks back" over all the earlier tokens and decides which ones are worth paying attention to. To make that work, every token gets boiled down to two things: a **Key**, which is like a label advertising "here's what I'm about," and a **Value**, which carries "here's the actual information I contribute." There's a third piece, the Query — that's what the current token uses to search through all the Keys — but for our purposes the Key and the Value are what matter. You don't need the math. You just need one fact: a token's Key and Value never change once they're computed. They're fixed.
