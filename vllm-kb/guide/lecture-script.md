...
To predict the next token, the model uses a mechanism called **attention** — each token "looks back" over the earlier tokens and decides which ones matter. To make that work, every token gets turned into three little vectors: a **Query**, meaning "what am I looking for"; a **Key**, meaning "here's what I'm about"; and a **Value**, meaning "here's the information I carry."

Let me make that concrete, because these four steps *are* the attention mechanism — formally, scaled dot-product attention. Step one: the current token's Query is compared against every earlier token's Key using a dot product, which measures how aligned they are — that gives a relevance score for each earlier token. Step two: those scores are divided by the square root of the key dimension; this scaling just keeps the numbers from blowing up as the vectors get longer, which would otherwise make the next step too spiky. Step three: a softmax turns the scores into attention weights that are all positive and sum to one — so a token that's highly relevant might get a weight of 0.8 while an irrelevant one gets 0.02. Step four: we take a weighted sum of all the earlier tokens' Values, using those weights. The result is called the context vector — a custom blend of the earlier values, leaning toward whichever tokens this one found most relevant. That context vector is the token "having read its context." In one line, attention is: softmax of Q times K-transpose over root-d-k, all times V. A few rounding-out details: a causal mask stops a token from attending to anything after it — future scores are set to minus infinity before the softmax, and that's exactly what makes the keys and values cacheable. This whole recipe also runs in parallel across many "heads," each with its own Q, K, and V, so the model can track several kinds of relationship at once. And after attention, that context vector flows through the rest of the transformer layer — a feed-forward network, plus residual connections and layer normalization that keep the numbers in a stable range — and the stack of layers eventually produces the next-token probabilities we met in Part 1.
...
Now, here's where the name "PagedAttention" really earns itself — because scattering the cache creates a new problem. Remember, the attention step needs to read *all* of a sequence's Keys and Values. But we've just deliberately spread them across blocks all over the GPU. So the attention **kernel** — the GPU code that does the math — has to follow the page table and read directly from those scattered blocks, without first copying them into one neat place. That paged-aware kernel is the actual "attention" part of PagedAttention. The block bookkeeping we just described is only half the trick; the kernel that reads scattered memory efficiently is the other half.

And this is exactly where FlashAttention fits in, because people often wonder how it relates. FlashAttention is the famous fast-attention trick — but it is *not* a competitor to PagedAttention; the two compose, each solving a different problem. FlashAttention is about the *math*: it computes attention without ever writing the giant N-by-N attention matrix to memory, streaming over the keys and values in tiles and keeping running softmax statistics on-chip in fast SRAM. That's what makes each attention step fast and avoids a quadratic memory blow-up. PagedAttention, by contrast, is about the *layout* — where the keys and values live, in scattered blocks with a page table. So vLLM's FlashAttention backend is *paged-aware*: it runs FlashAttention's tiled math, but instead of reading one contiguous buffer it follows each request's block table to gather tiles from the scattered blocks. You get FlashAttention's speed and PagedAttention's memory efficiency in the same kernel. And FlashAttention is actually just one of several interchangeable attention backends vLLM can pick at startup — that "set the backend to FlashAttention" step from the worker init — alongside others like FlashInfer, a Triton backend, and specialized ones for certain model families. It's the default on most NVIDIA GPUs, but it's swappable; they all read the same paged KV blocks underneath.

Let me unpack why the SRAM detail matters, because it rests on one hardware fact worth knowing: a GPU has several kinds of memory, and they differ in speed by orders of magnitude. At the top are registers — per-thread, kilobytes, the fastest. Then SRAM, the on-chip shared memory, around twenty megabytes total on an A100, running at roughly nineteen terabytes per second, sitting right next to the cores. Then HBM — that's the VRAM, the forty or eighty gigabytes we keep talking about — which is huge but runs at only one and a half to two terabytes per second, so about ten times slower than SRAM. And finally, off the GPU entirely, system RAM over PCIe, which is slower still. The crucial gap: SRAM is roughly ten times faster than HBM but about a thousand times smaller. And the cores can only do math on data that's in registers or SRAM — anything in HBM has to be hauled in first, and that hauling is the real cost. Now you can see why FlashAttention is such a win. Naïve attention writes the full N-by-N score matrix out to slow HBM, reads it back to do the softmax, then reads it yet again to weight the values — three round-trips over the slow memory, and the matrix grows quadratically with sequence length. FlashAttention instead loads tiles of keys and values into SRAM, computes the scores, a running softmax, and the weighted sum entirely on-chip, and only writes the final result back to HBM. The giant matrix never touches HBM at all. Since attention is memory-bound — bottlenecked by HBM traffic, not by arithmetic — cutting those round-trips is exactly what makes it faster and lets it handle long sequences without blowing up memory. And notice it's the same enemy vLLM fights everywhere: HBM bandwidth. PagedAttention fights to fit more into HBM, continuous batching fights to keep the cores fed, and FlashAttention fights to keep attention's working set in SRAM — three angles on the same bottleneck, which is why they pair so naturally.
...
Now let me go one level deeper into the real internal design, because this is the part worth understanding well. This is vLLM's V1 engine. Think of it as four wrapper layers around a core. First, the config — every knob for model, cache, and parallelism, built once and threaded everywhere. Second, the processor — it validates your input, tokenizes it, and packages it into what the code calls an EngineCoreRequest, carrying the token ids, the sampling parameters, a priority, and the arrival time. Third, the engine-core client — the handle the outside world uses to talk to the core; in simple offline use it's literally the core in the same process, but at scale it becomes a multiprocess client talking over ZMQ sockets. And fourth, the output processor, which turns the core's raw outputs back into the readable, detokenized text you actually see.

Inside that core sit the pieces that do the work. The Model Executor drives the forward passes — on one GPU it's a single-process executor with one Worker; on many GPUs it's a multiprocess executor with one Worker per GPU. Each Worker owns a model runner — which holds the sampler, the KV cache, and the forward-pass buffers — and an input batch holding the CPU-side buffers and the per-request block tables. Then there's the Scheduler, with its policy (first-come-first-served or priority) and its waiting and running queues. And the KV cache manager, the heart of paged attention, which maintains the free-block queue — a pool of hundreds of thousands of free blocks — and a dictionary mapping each request to its allocated blocks. That dictionary *is* the page table.

It's also worth knowing what happens at startup, before any request is served. Each GPU Worker does three things. It initializes the device — pinning a CUDA device, checking there's enough VRAM for the requested utilization fraction, and setting up the parallelism groups. It loads the model — building the architecture, loading the weights, switching to inference mode. And it initializes the KV cache — running a dummy profiling forward pass and taking a memory snapshot to compute exactly how many blocks fit, binding those KV tensors to the attention layers, and, unless you disable it, capturing CUDA graphs — pre-recorded sequences of GPU work that get replayed each step to cut launch overhead.

Now, a question people often ask at this point: how many CUDA cores does the model get when it loads? And the answer is subtle but important — it gets all of them, all the time, because cores are never allocated. Notice that every single startup step I just described is about *memory* — weights, activations, KV blocks. vLLM claims a slice of VRAM; it never claims cores. The CUDA cores — there are about seven thousand on an A100, about seventeen thousand on an H100, plus the Tensor Cores that do the actual matrix multiplies — are fixed silicon. They're shared dynamically: when a kernel runs, the GPU's scheduler hands its threads to whatever cores are free at that instant, and two requests in the same batch share the same cores in the same launch. There's no per-model core budget to set. And here's why that matters for understanding vLLM's whole philosophy: during decode, you generate one token at a time, which barely exercises those thousands of cores — but it still has to stream all the weights and KV through memory. So decode is memory-bandwidth-bound, not core-bound. The cores sit largely idle; memory is the bottleneck. That's the deep reason vLLM spends all its cleverness on fitting more into VRAM and on batching — to feed those otherwise-idle cores — rather than on the cores themselves.

And one detail about scheduling that explains a lot: in each step, decode requests are scheduled *first*, because they're already running and each needs just one more token. Then prefill requests are admitted from the waiting queue until the token budget for the step runs out. Every time a request needs space, the scheduler calls allocate_slots, which computes how many blocks are needed — ceiling of new tokens over sixteen — checks the pool, possibly evicts a low-priority request if memory is tight, and pops blocks off the free-block queue. That's the precise moment the page table gets written. One last note: all of this describes the V1 engine, which is the current default. Its big wins over the old V0 are that it can mix prefill and decode in a single step, and it runs the API server in a separate process from the engine so that tokenization and HTTP never block the GPU loop.

Those boxes are the engine's *logical* parts — so let's ground them in physical reality, and here I'll be precise, because it's worth getting right. A running vLLM is not one process; in V1 it's *several*, all on one host, talking to each other over ZMQ sockets. There's the API server process, which handles HTTP and does the tokenizing. There's the engine core process, which runs the scheduler and the KV cache manager in its busy loop. And there's one GPU worker process per GPU, each loading its shard of the weights, holding its KV blocks, and running the forward passes. The GPU side does the math; the CPU side orchestrates. So concretely, per the official architecture docs, a single-node deployment with four GPUs through tensor parallelism runs *six* processes: one API server, one engine core, and four GPU workers. They're almost always wrapped in a single Docker container — the official image is called vllm-openai, exposing an OpenAI-compatible API on port eight thousand — and it's one container per node, not one per GPU. To handle more traffic, you add replicas behind a load balancer; that's data parallelism, which adds one engine core per replica plus a coordinator process, and we'll get to it next.
